...
if { [file exists $out1] && [file size $out1] > 0 } {
  puts "PASS: basic write_verilog created non-empty file"
} else {
...
if { [file exists $out2] && [file size $out2] > 0 } {
  puts "PASS: write_verilog -include_pwr_gnd created non-empty file"
} else {
...
if { [file exists $out3] && [file size $out3] > 0 } {
  puts "PASS: write_verilog -remove_cells {} created non-empty file"
} else {
...
if { $sz2 >= $sz1 } {
  puts "PASS: pwr_gnd output is >= basic output size"
} else {
...
if { [file exists $out4] && [file size $out4] > 0 } {
  puts "PASS: write_verilog -sort created file (with deprecation warning)"
}
...
write_verilog $out5
puts "PASS: roundtrip write completed"
