...
LibertyPort *
LibertyBuilder::makePort(LibertyCell *cell, const char *name)
{
  LibertyPort *port =
      new LibertyPort(cell, name, false, -1, -1, false, nullptr);
  cell->addPort(port);
  return port;
}
...
LibertyBuilder::makeBusPortBit(ConcreteLibrary *library,
                               ...
                               int bit_index)
{
  char *bit_name = stringPrintTmp("%s%c%d%c", bus_name, library->busBrktLeft(),
                                  bit_index, library->busBrktRight());
...
ConcretePort *
LibertyBuilder::makePort(LibertyCell *cell, const char *bit_name, int bit_index)
{
  ConcretePort *port = new LibertyPort(cell, bit_name, false, bit_index,
                               ...
                               TimingArcAttrs *attrs)
{
  ...
  switch (attrs->timingType()) {
    ...
    case TimingType::combinational_fall:
      return makeCombinationalArcs(cell, from_port, to_port, related_out, false,
                                   true, attrs);
    case TimingType::combinational_rise:
      return makeCombinationalArcs(cell, from_port, to_port, related_out, true,
                                   false, attrs);
    ...
    case TimingType::three_state_enable:
      return makeTristateEnableArcs(cell, from_port, to_port, related_out, true,
                                    true, attrs);
                                      ...
                                      TimingArcAttrs *attrs)
{
  ...
  TimingArcSet *arc_set =
      makeTimingArcSet(cell, from_port, to_port, related_out,
  ...
  if (sense == TimingSense::unknown && func) {
    ...
    if (sense == TimingSense::none && to_port->direction()->isAnyTristate()) {
      // from_port is not an input to function, check tristate enable.
      FuncExpr *enable = to_port->tristateEnable();
      if (enable && enable->hasPort(from_port)) sense = TimingSense::non_unate;
    }
  }
  TimingModel *model;
  RiseFall *to_rf;
  switch (sense) {
    case TimingSense::positive_unate:
      if (to_rise) {
        ...
        if (model) makeTimingArc(arc_set, RiseFall::rise(), to_rf, model);
      }
      if (to_fall) {
        to_rf = RiseFall::fall();
        model = attrs->model(to_rf);
        if (model) makeTimingArc(arc_set, RiseFall::fall(), to_rf, model);
      }
      break;
...
LibertyBuilder::makeLatchDtoQArcs(LibertyCell *cell,
                                  ...
                                  TimingArcAttrs *attrs)
{
  TimingArcSet *arc_set = makeTimingArcSet(
      cell, from_port, to_port, related_out, TimingRole::latchDtoQ(), attrs);
  TimingModel *model;
  ...
  if (model) {
    RiseFall *from_rf =
        (sense == TimingSense::negative_unate) ? to_rf->opposite() : to_rf;
                                 ...
                                 TimingArcAttrs *attrs)
{
  ...
  while (port_iter.hasNext()) {
    ...
    if (seq) {
      if (seq->clock() && seq->clock()->hasPort(from_port)) {
        TimingRole *role = seq->isRegister() ? TimingRole::regClkToQ()
                                             : TimingRole::latchEnToQ();
        return makeFromTransitionArcs(cell, from_port, to_port, related_out,
                                      from_rf, role, attrs);
      } else if (seq->isLatch() && seq->data() &&
  ...
  return makeFromTransitionArcs(cell, from_port, to_port, related_out, from_rf,
...
{
  TimingArcSet *arc_set =
      makeTimingArcSet(cell, from_port, to_port, related_out, role, attrs);
  TimingModel *model;
  RiseFall *to_rf;
  to_rf = RiseFall::rise();
  model = attrs->model(to_rf);
  if (model) makeTimingArc(arc_set, from_rf, to_rf, model);
  to_rf = RiseFall::fall();
  model = attrs->model(to_rf);
  if (model) makeTimingArc(arc_set, from_rf, to_rf, model);
  return arc_set;
                                        ...
                                        TimingArcAttrs *attrs)
{
  TimingArcSet *arc_set =
      makeTimingArcSet(cell, from_port, to_port, related_out,
                       TimingRole::tristateDisable(), attrs);
  TimingSense sense = attrs->timingSense();
  FuncExpr *tristate_enable = to_port->tristateEnable();
  if (sense == TimingSense::unknown && tristate_enable)
...
LibertyBuilder::makeTimingArc(TimingArcSet *set,
                              ...
                              TimingModel *model)
{
  return new TimingArc(set, from_rf->asTransition(), to_rf->asTransition(),
                       model);
}
...
LeakagePower *
LibertyBuilder::makeLeakagePower(LibertyCell *cell, LeakagePowerAttrs *attrs)
{
