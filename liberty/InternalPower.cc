...
void
InternalPowerAttrs::setModel(RiseFall *rf, InternalPowerModel *model)
{
...
InternalPowerModel::InternalPowerModel(TableModel *model) : model_(model)
{
}
                          ...
                          float load_cap) const
{
  if (model_) {
    ...
    findAxisValues(in_slew, load_cap, axis_value1, axis_value2, axis_value3);
    const LibertyLibrary *library = cell->libertyLibrary();
    return model_->findValue(library, cell, pvt, axis_value1, axis_value2,
                             axis_value3);
  } else
    return 0.0;
}
                                ...
                                string *result) const
{
  if (model_) {
    ...
    findAxisValues(in_slew, load_cap, axis_value1, axis_value2, axis_value3);
    const LibertyLibrary *library = cell->libertyLibrary();
    model_->reportValue("Power", library, cell, pvt, axis_value1, nullptr,
...
InternalPowerModel::checkAxes(const TableModel *model)
{
  ...
  bool axis_ok = true;
  if (axis1) axis_ok &= checkAxis(model->axis1());
  if (axis2) axis_ok &= checkAxis(model->axis2());
