...
LibertyLibrary::LibertyLibrary(const char *name, const char *filename) :
    ...
    buffers_(nullptr)
{
  ...
  for (int i = 0; i != int(TableTemplateType::count); i++) {
    ...
    TableTemplate *scalar_template =
        new TableTemplate("scalar", nullptr, nullptr, nullptr);
                                         ...
                                         LibertyCellSeq *cells)
{
  ...
  while (cell_iter.hasNext()) {
    ...
    if (pattern->match(cell->name())) cells->push_back(cell);
  }
}
...
LibertyLibrary::buffers()
{
  if (buffers_ == nullptr) {
    ...
    while (cell_iter.hasNext()) {
      LibertyCell *cell = cell_iter.next();
      if (!cell->dontUse() && cell->isBuffer()) buffers_->push_back(cell);
    }
  }
  return buffers_;
}
...
TableTemplate *
LibertyLibrary::findTableTemplate(const char *name, TableTemplateType type)
{
...
float
LibertyLibrary::scaleFactor(ScaleFactorType type, const Pvt *pvt) const
{
...
LibertyLibrary::scaleFactor(ScaleFactorType type,
                            ...
                            const Pvt *pvt) const
{
  if (pvt == nullptr) pvt = default_operating_conditions_;
  ...
  if (pvt) {
    ...
    if (cell) scale_factors = cell->scaleFactors();
    if (scale_factors == nullptr) scale_factors = scale_factors_;
    if (scale_factors) {
      float process_scale = 1.0F +
          (pvt->process() - nominal_process_) *
              scale_factors->scale(type, ScaleFactorPvt::process, tr_index);
      float temp_scale = 1.0F +
          (pvt->temperature() - nominal_temperature_) *
              scale_factors->scale(type, ScaleFactorPvt::temp, tr_index);
      float volt_scale = 1.0F +
          (pvt->voltage() - nominal_voltage_) *
...
void
LibertyLibrary::setWireSlewDegradationTable(TableModel *model, RiseFall *rf)
{
...
void
LibertyLibrary::defaultMaxFanout(float &fanout, bool &exists) const
{
...
void
LibertyLibrary::defaultMaxSlew(float &slew, bool &exists) const
{
...
void
LibertyLibrary::defaultMaxCapacitance(float &cap, bool &exists) const
{
...
void
LibertyLibrary::setDefaultIntrinsic(const RiseFall *rf, float value)
{
...
void
LibertyLibrary::setDefaultBidirectPinRes(const RiseFall *rf, float value)
{
...
void
LibertyLibrary::setDefaultOutputPinRes(const RiseFall *rf, float value)
{
...
void
LibertyLibrary::setInputThreshold(const RiseFall *rf, float th)
{
...
void
LibertyLibrary::setOutputThreshold(const RiseFall *rf, float th)
{
...
void
LibertyLibrary::setSlewLowerThreshold(const RiseFall *rf, float th)
{
...
void
LibertyLibrary::setSlewUpperThreshold(const RiseFall *rf, float th)
{
...
LibertyCell *
LibertyLibrary::makeScaledCell(const char *name, const char *filename)
{
                              ...
                              Report *report)
{
  ...
  while (cell_iter.hasNext()) {
    ...
    if (link_cell) makeCornerMap(link_cell, cell, ap_index, report);
  }
}
                              ...
                              Report *report)
{
  ...
  while (port_iter1.hasNext()) {
    ...
    if (port2) {
      if (link) port1->setCornerPort(port2, ap_index);
    } else
      report->warn("cell %s/%s port %s not found in cell %s/%s.\n",
                   cell1->library()->name(), cell1->name(), port_name,
                   cell2->library()->name(), cell2->name());
  }
    ...
    if (arc_set2) {
      if (link) {
        ...
        while (arc_iter1.hasNext() && arc_iter2.hasNext()) {
          ...
          if (TimingArc::equiv(arc1, arc2)) arc1->setCornerArc(arc2, ap_index);
        }
      }
    } else
      report->warn(
          "cell %s/%s %s -> %s timing group %s not found in cell %s/%s.\n",
          cell1->library()->name(), cell1->name(), arc_set1->from()->name(),
          arc_set1->to()->name(), arc_set1->role()->asString(),
          cell2->library()->name(), cell2->name());
  }
}
...
void
LibertyLibrary::addSupplyVoltage(const char *supply_name, float voltage)
{
                                      ...
                                      LibertyPortSeq *ports) const
{
  ...
  while (port_iter.hasNext()) {
    ...
    if (pattern->match(port->name())) ports->push_back(port);
  }
}
...
LibertyCell::addPort(ConcretePort *port)
{
  ...
  if (port->direction()->isInternal()) has_internal_ports_ = true;
}
                           ...
                           const LibertyPort *output) const
{
  ...
  return func && func->op() == FuncExpr::op_port && func->port() == input;
}
...
LibertyCell::addTimingArcSet(TimingArcSet *arc_set)
{
  ...
  if (role == TimingRole::regClkToQ() || role == TimingRole::latchEnToQ())
    from->setIsRegClk(true);
  if (role->isTimingCheck()) from->setIsCheckClk(true);
  return set_index;
}
...
void
LibertyCell::finish(bool infer_latches, Report *report, Debug *debug)
{
  ...
  if (infer_latches && !interface_timing_) inferLatchRoles(debug);
}
...
LibertyCell::findDefaultCondArcs()
{
  for (auto port_pair_set : port_timing_arc_set_map_) {
    ...
    if (has_cond_arcs) {
      for (auto set : *sets) {
        if (!set->cond()) set->setIsCondDefault(true);
      }
    }
  }
}
...
TimingArcSetSeq *
LibertyCell::timingArcSets(const LibertyPort *from, const LibertyPort *to) const
{
  if (from && to) {
    LibertyPortPair port_pair(from, to);
                            ...
                            LibertyPort *output_inv)
{
  for (int bit = 0; bit < size; bit++) {
    ...
    if (clk) clk_bit = clk->bitSubExpr(bit);
    FuncExpr *data_bit = nullptr;
    if (data) data_bit = data->bitSubExpr(bit);
    FuncExpr *clear_bit = nullptr;
    if (clear) clear_bit = clear->bitSubExpr(bit);
    FuncExpr *preset_bit = nullptr;
    if (preset) preset_bit = preset->bitSubExpr(bit);
    ...
    Sequential *seq = new Sequential(is_register, clk_bit, data_bit, clear_bit,
                                     preset_bit, clr_preset_out,
                                     clr_preset_out_inv, out_bit, out_inv_bit);
...
void
LibertyCell::setCornerCell(LibertyCell *corner_cell, int ap_index)
{
  if (ap_index >= static_cast<int>(corner_cells_.size()))
...
void
LibertyCell::makeLatchEnables(Report *report, Debug *debug)
{
  if (hasSequentials() || hasInferedRegTimingArcs()) {
          ...
          if (d_to_q->role() == TimingRole::latchDtoQ()) {
            ...
            while (check_iter.hasNext()) {
              ...
              if (setup_check->role() == TimingRole::setup()) {
                LatchEnable *latch_enable = makeLatchEnable(
                    d, en, q, d_to_q, en_to_q, setup_check, debug);
                ...
                if (check_arc_iter.hasNext()) {
                  ...
                  if (check_rf == en_rf) {
                    report->warn(
                        "cell %s/%s %s -> %s latch enable %s_edge timing arc "
                        "is inconsistent with %s -> %s setup_%s check.\n",
                        library_->name(), name_, en->name(), q->name(),
                  ...
                  if (en_func) {
                    ...
                    if (en_sense == TimingSense::positive_unate &&
                        en_rf != RiseFall::rise())
                      report->warn(
                          "cell %s/%s %s -> %s latch enable %s_edge is "
                          "inconsistent with latch group enable function "
                          "positive sense.\n",
                          library_->name(), name_, en->name(), q->name(),
                          en_rf == RiseFall::rise() ? "rising" : "falling");
...
FuncExpr *
LibertyCell::findLatchEnableFunc(LibertyPort *data, LibertyPort *enable) const
{
  for (auto seq : sequentials_) {
    if (seq->isLatch() && seq->data() && seq->data()->hasPort(data) &&
        seq->clock() && seq->clock()->hasPort(enable))
      return seq->clock();
  }
  return nullptr;
}
                             ...
                             Debug *debug)
{
  ...
  LatchEnable *latch_enable =
      new LatchEnable(d, en, en_rf, en_func, q, d_to_q, en_to_q, setup_check);
  // Multiple enables for D->Q pairs are not supported.
  if (latch_d_to_q_map_[d_to_q]) delete latch_d_to_q_map_[d_to_q];
...
{
  if (function_) function_->deleteSubexprs();
  if (tristate_enable_) tristate_enable_->deleteSubexprs();
...
LibertyPort::setDirection(PortDirection *dir)
{
  ...
  if (dir->isInternal()) liberty_cell_->setHasInternalPorts(true);
}
...
float
LibertyPort::capacitance(const RiseFall *rf, const MinMax *min_max) const
{
  float cap;
  bool exists;
                         ...
                         const Pvt *pvt) const
{
  if (scaled_ports_) {
    ...
    // nominal pvt.
    if (scaled_port) return scaled_port->capacitance(rf, min_max);
  }
...
float
LibertyPort::driveResistance(const RiseFall *rf, const MinMax *min_max) const
{
  ...
  while (set_iter.hasNext()) {
    ...
    if (!set->role()->isTimingCheck()) {
      ...
      while (arc_iter.hasNext()) {
        ...
        if (rf == nullptr || arc->toTrans()->asRiseFall() == rf) {
          GateTimingModel *model =
              dynamic_cast<GateTimingModel *>(arc->model());
          if (model) {
            float drive = model->driveResistance(liberty_cell_, nullptr);
            if (min_max->compare(drive, max_drive)) max_drive = drive;
            found_drive = true;
          }
        }
      }
...
void
LibertyPort::setCapacitanceLimit(float cap, const MinMax *min_max)
{
...
void
LibertyPort::setFanoutLimit(float fanout, const MinMax *min_max)
{
                       ...
                       bool &exists) const
{
  ...
  min_period = min_period_ *
      lib->scaleFactor(ScaleFactorType::min_period, liberty_cell_, pvt);
  exists = min_period_exists_;
}

void
LibertyPort::minPeriod(float &min_period, bool &exists) const
{
  min_period = min_period_;
  exists = min_period_exists_;
}
...
void
LibertyPort::setMinPulseWidth(RiseFall *hi_low, float min_width)
{
  int hi_low_index = hi_low->index();
...
bool
LibertyPort::equiv(const LibertyPort *port1, const LibertyPort *port2)
{
...
bool
LibertyPort::less(const LibertyPort *port1, const LibertyPort *port2)
{
...
{
  if (scaled_ports_ == nullptr) scaled_ports_ = new ScaledPortMap;
...
void
LibertyPort::setPulseClk(RiseFall *trigger, RiseFall *sense)
{
...
LibertyPort::cornerPort(int ap_index)
{
  if (ap_index < static_cast<int>(corner_ports_.size())) {
    ...
    if (corner_port) return corner_port;
  }
  return this;
}

void
LibertyPort::setCornerPort(LibertyPort *corner_port, int ap_index)
{
  if (ap_index >= static_cast<int>(corner_ports_.size()))
...
sortLibertyPortSet(LibertyPortSet *set, LibertyPortSeq &ports)
{
  ...
  while (port_iter.hasNext()) ports.push_back(port_iter.next());
...
BusDcl::BusDcl(const char *name, int from, int to) :
...
ModeDef::ModeDef(const char *name) : name_(stringCopy(name))
{
}
...
ModeValueDef *
ModeDef::defineValue(const char *value, FuncExpr *cond, const char *sdf_cond)
{
...
Pvt::Pvt(float process, float voltage, float temperature) :
    process_(process),
    voltage_(voltage),
...
{
  return type == ScaleFactorType::cell || type == ScaleFactorType::hold ||
      type == ScaleFactorType::setup || type == ScaleFactorType::recovery ||
      type == ScaleFactorType::removal || type == ScaleFactorType::nochange ||
...
ScaleFactors::ScaleFactors(const char *name) : name_(stringCopy(name))
{
...
void
ScaleFactors::setScale(ScaleFactorType type, ScaleFactorPvt pvt, float scale)
{
  scales_[int(type)][int(pvt)][0] = scale;
}

float
ScaleFactors::scale(ScaleFactorType type, ScaleFactorPvt pvt, RiseFall *rf)
{
  return scales_[int(type)][int(pvt)][rf->index()];
}

float
ScaleFactors::scale(ScaleFactorType type, ScaleFactorPvt pvt, int tr_index)
{
  return scales_[int(type)][int(pvt)][tr_index];
}

float
ScaleFactors::scale(ScaleFactorType type, ScaleFactorPvt pvt)
{
  ...
  for (int type_index = 0; type_index < scale_factor_type_count; type_index++) {
    ...
    for (int pvt_index = 0; pvt_index < int(ScaleFactorPvt::count);
      ...
      if (scaleFactorTypeRiseFallSuffix(type) ||
        ...
        printf(" %.3f", scales_[type_index][pvt_index][0]);
      }
    }
    printf("\n");
  }
...
OcvDerate::OcvDerate(const char *name) : name_(name)
{
...
OcvDerate::~OcvDerate()
{
  ...
  for (auto el_index : EarlyLate::rangeIndex()) {
    for (auto tr_index : RiseFall::rangeIndex()) {
      ...
      if (derate) models.insert(derate);
...
LibertyPgPort::LibertyPgPort(const char *name, LibertyCell *cell) :
    name_(stringCopy(name)),
    pg_type_(unknown),
