...
reportPvt(const LibertyLibrary *library,
          ...
          string *result);
static void
appendSpaces(string *result, int count);
...
{
  if (delay_model_) delay_model_->setIsScaled(is_scaled);
  if (slew_model_) slew_model_->setIsScaled(is_scaled);
}
                          ...
                          Slew &drvr_slew) const
{
  ...
  float delay = findValue(library, cell, pvt, delay_model_, in_slew, load_cap,
                          related_out_cap);
  ...
  if (pocv_enabled && delay_sigma_models_[EarlyLate::lateIndex()])
    sigma_late = findValue(library, cell, pvt,
                           delay_sigma_models_[EarlyLate::lateIndex()], in_slew,
                           load_cap, related_out_cap);
  gate_delay = makeDelay(delay, sigma_early, sigma_late);

  float slew = findValue(library, cell, pvt, slew_model_, in_slew, load_cap,
                         related_out_cap);
  ...
  if (pocv_enabled && slew_sigma_models_[EarlyLate::lateIndex()])
    sigma_late = findValue(library, cell, pvt,
                           slew_sigma_models_[EarlyLate::lateIndex()], in_slew,
                           load_cap, related_out_cap);
  // Clip negative slews to zero.
  if (slew < 0.0) slew = 0.0;
                                ...
                                string *result) const
{
  ...
  if (pocv_enabled && delay_sigma_models_[EarlyLate::earlyIndex()])
    reportTableLookup("Delay sigma(early)", library, cell, pvt,
                      delay_sigma_models_[EarlyLate::earlyIndex()], in_slew,
                      load_cap, related_out_cap, digits, result);
  if (pocv_enabled && delay_sigma_models_[EarlyLate::lateIndex()])
    reportTableLookup("Delay sigma(late)", library, cell, pvt,
                      ...
                      load_cap, related_out_cap, digits, result);
  *result += '\n';
  reportTableLookup("Slew", library, cell, pvt, slew_model_, in_slew, load_cap,
                    related_out_cap, digits, result);
  if (pocv_enabled && slew_sigma_models_[EarlyLate::earlyIndex()])
    reportTableLookup("Slew sigma(early)", library, cell, pvt,
                      slew_sigma_models_[EarlyLate::earlyIndex()], in_slew,
                      load_cap, related_out_cap, digits, result);
  if (pocv_enabled && slew_sigma_models_[EarlyLate::lateIndex()])
                                  ...
                                  string *result) const
{
  if (model) {
    ...
    model->reportValue(result_name, library, cell, pvt, axis_value1, nullptr,
                       axis_value2, axis_value3, digits, result);
  }
}
                          ...
                          float related_out_cap) const
{
  if (model) {
    ...
    findAxisValues(model, in_slew, load_cap, related_out_cap, axis_value1,
                   axis_value2, axis_value3);
    return model->findValue(library, cell, pvt, axis_value1, axis_value2,
                            axis_value3);
  } else
    return 0.0;
}
                               ...
                               float &axis_value3) const
{
  switch (model->order()) {
    ...
    case 1:
      axis_value1 =
          axisValue(model->axis1(), in_slew, load_cap, related_out_cap);
      axis_value2 = 0.0;
      axis_value3 = 0.0;
      break;
    case 2:
      axis_value1 =
          axisValue(model->axis1(), in_slew, load_cap, related_out_cap);
      axis_value2 =
          axisValue(model->axis2(), in_slew, load_cap, related_out_cap);
      axis_value3 = 0.0;
      break;
    case 3:
      axis_value1 =
      ...
      axis_value2 =
          axisValue(model->axis2(), in_slew, load_cap, related_out_cap);
      axis_value3 =
          axisValue(model->axis3(), in_slew, load_cap, related_out_cap);
      break;
    default:
...
float
GateTableModel::driveResistance(const LibertyCell *cell, const Pvt *pvt) const
{
  float slew, cap;
                           ...
                           float &cap) const
{
  ...
  if (axis1 &&
    ...
    slew = findValue(library, cell, pvt, slew_model_, in_slew, cap, 0.0);
  } else if (axis2 &&
             axis2->variable() ==
...
GateTableModel::checkAxes(const Table *table)
{
  ...
  bool axis_ok = true;
  if (axis1) axis_ok &= checkAxis(table->axis1());
  if (axis2) axis_ok &= checkAxis(table->axis2());
  if (axis3) axis_ok &= checkAxis(table->axis3());
  return axis_ok;
}
                            ...
                            ArcDelay &margin) const
{
  if (model_) {
    const LibertyLibrary *library = cell->libertyLibrary();
    float mean = findValue(library, cell, pvt, model_, from_slew, to_slew,
                           related_out_cap);
    float sigma_early = 0.0;
    float sigma_late = 0.0;
    if (pocv_enabled && sigma_models_[EarlyLate::earlyIndex()])
      sigma_early =
          findValue(library, cell, pvt, sigma_models_[EarlyLate::earlyIndex()],
                           ...
                           float related_out_cap) const
{
  if (model) {
    float axis_value1, axis_value2, axis_value3;
    findAxisValues(from_slew, to_slew, related_out_cap, axis_value1,
                   axis_value2, axis_value3);
    return model->findValue(library, cell, pvt, axis_value1, axis_value2,
                            axis_value3);
  } else
...
{
  const LibertyLibrary *library = cell->libertyLibrary();
  reportTableDelay("Check", library, cell, pvt, model_, from_slew,
                   from_slew_annotation, to_slew, related_out_cap, digits,
                   result);
  if (pocv_enabled && sigma_models_[EarlyLate::earlyIndex()])
    reportTableDelay("Check sigma early", library, cell, pvt,
                     sigma_models_[EarlyLate::earlyIndex()], from_slew,
                     from_slew_annotation, to_slew, related_out_cap, digits,
                     result);
  if (pocv_enabled && sigma_models_[EarlyLate::lateIndex()])
    reportTableDelay("Check sigma late", library, cell, pvt,
                     sigma_models_[EarlyLate::lateIndex()], from_slew,
                     from_slew_annotation, to_slew, related_out_cap, digits,
                     result);
}
                                  ...
                                  string *result) const
{
  if (model) {
    ...
    findAxisValues(from_slew, to_slew, related_out_cap, axis_value1,
                   axis_value2, axis_value3);
    reportPvt(library, pvt, digits, result);
    model_->reportValue(result_name, library, cell, pvt, axis_value1,
                        from_slew_annotation, axis_value2, axis_value3, digits,
                        result);
  }
}
                                ...
                                float &axis_value3) const
{
  switch (model_->order()) {
    ...
    case 1:
      axis_value1 =
          axisValue(model_->axis1(), from_slew, to_slew, related_out_cap);
      axis_value2 = 0.0;
      axis_value3 = 0.0;
      break;
    case 2:
      axis_value1 =
          axisValue(model_->axis1(), from_slew, to_slew, related_out_cap);
      axis_value2 =
          axisValue(model_->axis2(), from_slew, to_slew, related_out_cap);
      axis_value3 = 0.0;
      break;
    case 3:
      axis_value1 =
      ...
      axis_value2 =
          axisValue(model_->axis2(), from_slew, to_slew, related_out_cap);
      axis_value3 =
          axisValue(model_->axis3(), from_slew, to_slew, related_out_cap);
      break;
    default:
      internalError("unsupported table order");
  }
                        ...
                        const Pvt *pvt) const
{
  if (is_scaled_)
    ...
    return library->scaleFactor(
        static_cast<ScaleFactorType>(scale_factor_type_), tr_index_, cell, pvt);
}
...
TableModel::reportValue(const char *result_name,
                        ...
                        string *result) const
{
  table_->reportValue("Table value", library, cell, pvt, value1, comment1,
                      value2, value3, digits, result);
  ...
  *result += table_unit->asString(
      findValue(library, cell, pvt, value1, value2, value3), digits);
  *result += '\n';
}
...
reportPvt(const LibertyLibrary *library,
          ...
          string *result)
{
  if (pvt == nullptr) pvt = library->defaultOperatingConditions();
  if (pvt) {
    string pvt_str;
    stringPrint(pvt_str, "P = %.*f V = %.*f T = %.*f\n", digits, pvt->process(),
                digits, pvt->voltage(), digits, pvt->temperature());
    *result += pvt_str;
  }
}
...
Table0::Table0(float value) : Table(), value_(value)
{
}

float
Table0::findValue(float, float, float) const
{
  return value_;
}
                    ...
                    string *result) const
{
  ...
  *result += table_unit->asString(findValue(value1, value2, value3), digits);
  if (comment1) *result += comment1;
  *result += '\n';
}

void
Table0::report(const Units *units, Report *report) const
{
  int digits = 4;
  const Unit *table_unit = units->timeUnit();
...
Table1::Table1(FloatSeq *values, TableAxis *axis1, bool own_axis1) :
    Table(),
    values_(values),
    axis1_(axis1),
...
float
Table1::findValue(float value1, float, float) const
{
  if (axis1_->size() == 1)
                    ...
                    string *result) const
{
  ...
  *result += '\n';
  ...
  if (axis1_->size() != 1) {
    ...
    *result += "     ";
    *result += table_unit->asString(tableValue(index1 + 1), digits);
    *result += '\n';
  }
...
void
Table1::report(const Units *units, Report *report) const
{
  int digits = 4;
...
Table2::~Table2()
{
  ...
  delete values_;
  if (own_axis1_) delete axis1_;
  if (own_axis2_) delete axis2_;
}

float
Table2::tableValue(size_t index1, size_t index2) const
{
...
float
Table2::findValue(float value1, float value2, float) const
{
  ...
  if (size1 == 1) {
    if (size2 == 1)
      ...
      float tbl_value = (1 - dx2) * y00 + dx2 * y01;
      return tbl_value;
    }
  } else if (size2 == 1) {
    size_t index1 = axis1_->findAxisIndex(value1);
    ...
    float tbl_value = (1 - dx1) * y00 + dx1 * y10;
    return tbl_value;
  } else {
    size_t index1 = axis1_->findAxisIndex(value1);
    size_t index2 = axis2_->findAxisIndex(value2);
    ...
    float tbl_value = (1 - dx1) * (1 - dx2) * y00 + dx1 * (1 - dx2) * y10 +
                    ...
                    string *result) const
{
  ...
  *result += "------- ";
  *result += tableVariableString(var1), *result += " = ";
  *result += unit1->asString(value1, digits);
  if (comment1) *result += comment1;
  *result += '\n';
  ...
  if (axis1_->size() != 1) {
    ...
    *result += table_unit->asString(tableValue(index1 + 1, index2), digits);
    if (axis2_->size() != 1) {
      *result += "     ";
      *result +=
          table_unit->asString(tableValue(index1 + 1, index2 + 1), digits);
    }
  }
  *result += '\n';
...
void
Table2::report(const Units *units, Report *report) const
{
  int digits = 4;
  const Unit *table_unit = units->timeUnit();
  const Unit *unit1 = tableVariableUnit(axis1_->variable(), units);
  ...
  for (size_t index1 = 0; index1 < axis1_->size(); index1++) {
    ...
    for (size_t index2 = 0; index2 < axis2_->size(); index2++)
      report->print("%s ",
                    table_unit->asString(tableValue(index1, index2), digits));
    report->print("\n");
  }
}
...
Table3::~Table3()
{
  if (own_axis3_) delete axis3_;
}

float
Table3::tableValue(size_t index1, size_t index2, size_t index3) const
{
...
// Bilinear Interpolation.
float
Table3::findValue(float value1, float value2, float value3) const
{
  size_t index1 = axis1_->findAxisIndex(value1);
  size_t index2 = axis2_->findAxisIndex(value2);
  ...
  if (axis1_->size() != 1) {
    ...
    if (axis3_->size() != 1) y101 = tableValue(index1 + 1, index2, index3 + 1);
    if (axis2_->size() != 1) {
  ...
  if (axis2_->size() != 1) {
    ...
    if (axis3_->size() != 1) y011 = tableValue(index1, index2 + 1, index3 + 1);
  }
  if (axis3_->size() != 1) {
  ...
  float tbl_value = (1 - dx1) * (1 - dx2) * (1 - dx3) * y000 +
                    ...
                    string *result) const
{
  ...
  *result += "   |    ---- ";
  *result += tableVariableString(var2), *result += " = ";
  *result += unit2->asString(value2, digits);
  *result += '\n';
  ...
  *result += "   v    |    --------------------\n";
  ...
  if (axis1_->size() != 1) {
    ...
    *result += "   v   / ";
    *result +=
        table_unit->asString(tableValue(index1 + 1, index2, index3), digits);
    if (axis3_->size() != 1) {
      *result += "     ";
      *result += table_unit->asString(
          tableValue(index1 + 1, index2, index3 + 1), digits);
    }
  } else {
  ...
  *result += table_unit->asString(tableValue(index1, index2, index3), digits);
  if (axis3_->size() != 1) {
    *result += "     ";
    *result +=
        table_unit->asString(tableValue(index1, index2, index3 + 1), digits);
  }
  *result += '\n';
  ...
  if (axis1_->size() != 1 && axis2_->size() != 1) {
    *result += table_unit->asString(tableValue(index1 + 1, index2 + 1, index3),
    ...
    if (axis3_->size() != 1) {
      *result += "     ";
      *result += table_unit->asString(
          tableValue(index1 + 1, index2 + 1, index3 + 1), digits);
    }
  }
  *result += '\n';
  ...
  *result += " | ";
  if (axis2_->size() != 1) {
    *result +=
        table_unit->asString(tableValue(index1, index2 + 1, index3), digits);
    if (axis3_->size() != 1) {
      *result += "     ";
      *result += table_unit->asString(
          tableValue(index1, index2 + 1, index3 + 1), digits);
    }
  }
  *result += '\n';
...
void
Table3::report(const Units *units, Report *report) const
{
  int digits = 4;
  const Unit *table_unit = units->timeUnit();
  const Unit *unit1 = tableVariableUnit(axis1_->variable(), units);
  ...
  for (size_t index1 = 0; index1 < axis1_->size(); index1++) {
    ...
    for (size_t index2 = 0; index2 < axis2_->size(); index2++) {
      ...
      for (size_t index3 = 0; index3 < axis3_->size(); index3++)
        report->print(
            "%s ",
            table_unit->asString(tableValue(index1, index2, index3), digits));
      report->print("\n");
    }
  }
}
...
TableAxis::TableAxis(TableAxisVariable variable, FloatSeq *values) :
    variable_(variable),
    values_(values)
{
}
...
const Unit *
tableVariableUnit(TableAxisVariable variable, const Units *units)
{
  switch (variable) {
