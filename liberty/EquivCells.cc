...
cellDriveResistance(const LibertyCell *cell)
{
  ...
  while (port_iter.hasNext()) {
    ...
    if (port->direction()->isOutput()) return port->driveResistance();
  }
  return 0.0;
}
...
{
 public:
  bool operator()(const LibertyCell *cell1, const LibertyCell *cell2) const
  {
                       ...
                       LibertyLibrarySeq *map_libs)
{
  ...
  for (auto lib : *equiv_libs) findEquivCells(lib, hash_matches);
  ...
  if (map_libs) {
    for (auto lib : *map_libs) mapEquivCells(lib, hash_matches);
  }
...
{
  for (auto cell : unique_equiv_cells_) delete equiv_cells_.findKey(cell);
}
...
hashPort(const LibertyPort *port)
{
  return hashString(port->name()) * 3 + port->direction()->index() * 5;
}
...
bool
equivCells(const LibertyCell *cell1, const LibertyCell *cell2)
{
...
bool
equivCellPortsAndFuncs(const LibertyCell *cell1, const LibertyCell *cell2)
{
...
bool
equivCellPorts(const LibertyCell *cell1, const LibertyCell *cell2)
{
  if (cell1->portCount() != cell2->portCount())
    return false;
  else {
    ...
    while (port_iter1.hasNext()) {
      ...
      const char *name = port1->name();
      LibertyPort *port2 = cell2->findLibertyPort(name);
      if (!(port2 && LibertyPort::equiv(port1, port2))) return false;
    }
    return true;
  }
}
...
bool
equivCellSequentials(const LibertyCell *cell1, const LibertyCell *cell2)
{
...
bool
equivCellTimingArcSets(const LibertyCell *cell1, const LibertyCell *cell2)
{
  if (cell1->timingArcSetCount() != cell2->timingArcSetCount())
    ...
    while (set_iter1.hasNext()) {
      ...
      if (!(set2 && TimingArcSet::equiv(set1, set2))) return false;
    }
    return true;
  }
}
